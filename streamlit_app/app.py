...
def get_predictions_from_mongodb():
    """Get latest predictions from MongoDB cloud"""
    try:
        db = get_db_handler()
        predictions_doc = db.db.predictions.find_one(sort=[('timestamp', -1)])
        if predictions_doc and 'predictions' in predictions_doc:
            return predictions_doc['predictions']
        return None
    except Exception as e:
        return None
...
def main():
    ...
    try:
        with st.spinner("Loading..."):
            ...
            st.info(f"Predictions generated by: {model_details[model_choice]['name']}")
